...
The library includes two different fast generators of uniformly distributed pseudo-random numbers: MWC256 and Xoshiro256+, which are known to pass the relevant statistical tests.[12] There is a ziggurat algorithm, coded by J. A. Doornik,[9] for obtaining random numbers with a Gaussian distribution.
...
If you use `"Xoshiro256+"`, you can perform "long jumps" using `void RanJump_xoshiro256p(uint64_t uJumps)`. Each of the `uJumps` long jumps forwards the PRNG by 2^192 steps, giving access to an independent stream of random numbers.
...
- Include programs that explicitly test final quality of randomness of generated normally distributed random numbers. See, e.g., [8] for feeding output to standard random test suites.
...
- [Kschischang](https://www.comm.utoronto.ca/~frank/ZMG/) has made a very nicely documented and well-structured (yet platform-dependent) C implementation of McFarland's 2016 [10] ziggurat algorithm. A copy of Kschischang's code package (`zmg-0.90`) is located in the folder `zmg_kschischang`. In a preliminary (unoptimized) test on Windows (gcc, w64devkit), ZMG (w/ PCG64) performed approx. 2.5x slower than ZIGNOR (w/ MWC256 or Xoshiro256+) for generation of large numbers of normally distributed random numbers.
